...
if (isset($_GET['reactivar'])) {
    ...
    $stmt->execute();
    $stmt->close();
    header("Location: buscar_beneficiarios.php");
    exit();
}
...
// Búsqueda (activos y desactivos)
$busqueda = "";
if (isset($_GET['buscar'])) {
    $busqueda = $_GET['buscar'];
    $sql = "SELECT * FROM beneficiarios WHERE nombre LIKE ? OR apellido LIKE ? OR cedula LIKE ? ORDER BY fecha_registro DESC";
...
<div class="container">
    ...
    <form method="get" action="buscar_beneficiarios.php" class="buscar-form">
        ...
        <a href="buscar_beneficiarios.php" class="mostrar-todos"><i class="fa-solid fa-list"></i> Mostrar todos</a>
    </form>
    <h3><i class="fa-solid fa-magnifying-glass"></i> Beneficiarios (Activos y Desactivados)</h3>
    <table>
        <tr>
            <th>#</th>
            <th>Nombre</th>
        ...
        <form method="post" action="buscar_beneficiarios.php">
        <tr>
            <td><?= $contador ?></td>
            ...
            <td>
                ...
                <a href="buscar_beneficiarios.php" class="cancelar"><i class="fa-solid fa-xmark"></i></a>
            </td>
        </tr>
        </form>
        <?php else: ?>
        <tr>
            ...
            <td>
                <?php if ($row['activo'] == 1): ?>
                    <a href="buscar_beneficiarios.php?edit=<?= $row['id'] ?>" class="editar" title="Editar"><i class="fa-solid fa-pen-to-square"></i></a>
                <?php else: ?>
                    <a href="buscar_beneficiarios.php?reactivar=<?= $row['id'] ?>" class="reactivar" title="Reactivar" onclick="return confirm('¿Seguro que deseas reactivar este beneficiario?');"><i class="fa-solid fa-user-check"></i></a>
                <?php endif; ?>
            </td>
        </tr>
        <?php endif; ?>
